...
export function bachelier(type, F, K, sigma, T, df = 1) {
...
export function black76(type, F, K, sigma, T, df = 1) {
    if (F <= 0 || K <= 0)
        throw new Error("black76 requires positive F and K; use bachelier for power");
    const sd = sigma * Math.sqrt(T);
    const d1 = (Math.log(F / K) + 0.5 * sd * sd) / sd;
    const d2 = d1 - sd;
    if (type === "call")
        return { price: df * (F * normCdf(d1) - K * normCdf(d2)), delta: df * normCdf(d1) };
    return { price: df * (K * normCdf(-d2) - F * normCdf(-d1)), delta: -df * normCdf(-d1) };
}
/** Bought cap = call; bought floor = put (on price). */
export const cap = (F, K, sigma, T, df = 1) => bachelier("call", F, K, sigma, T, df);
export const floor = (F, K, sigma, T, df = 1) => bachelier("put", F, K, sigma, T, df);
/**
 ...
 */
export function zeroCostCollarFloor(F, capStrike, sigma, T, df = 1) {
