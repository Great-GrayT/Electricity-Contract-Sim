...
export class ReplaySession {
    ...
    marketP = []; // every finite market price (one per period)
    paidP = []; // ALL-IN effective price paid per period (incl. instruments)
    paidW = []; // load MWh weight for that effective price
    ...
    get paidCount() { return this.paidP.length; }
    ...
    priceHistograms(nbins = 30) {
        ...
        let paidTot = 0;
        for (let k = 0; k < this.paidP.length; k++) {
            paid[idx(this.paidP[k])] += this.paidW[k];
            paidTot += this.paidW[k];
        }
