...
export interface PriceModel {
  seasonal: number[][]; // [12][24] month x hour mean price
  globalMean: number;
  phiX: number; sigmaX: number;
  jumpIntensity: number; jumpMean: number; jumpStd: number; // per period, on X
  phiY: number; sigmaY: number;
  dtHours: number;
...
export function calibrate(ds: Dataset, opts: { jumpK?: number } = {}): { model: PriceModel; state0: ModelState } {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!isNum(p) || !isNum(e)) continue;
    const [m, h] = monthHour(e);
    sSum[m]![h]! += p; sCnt[m]![h]!++;
  }
  const globalMean = mean(price);
  const seasonal = Array.from({ length: 12 }, (_, m) =>
    Float64Array.from({ length: 24 }, (_, h) => (sCnt[m]![h]! ? sSum[m]![h]! / sCnt[m]![h]! : globalMean)),
  ...
  for (let i = 0; i < n; i++) {
    const p = price[i]!, e = epoch[i]!;
    if (!isNum(p) || !isNum(e)) { resid[i] = NaN; continue; }
    const [m, h] = monthHour(e);
  ...
  for (let i = 0; i < n; i++) X[i] = isNum(resid[i]!) ? resid[i]! - Y[i]! : NaN;
  ...
  for (let i = 1; i < n; i++) {
    const a = X[i]!, b = X[i - 1]!;
    if (isNum(a) && isNum(b)) innov.push(a - fitX.phi * b);
  }
  const innovStd = std(Float64Array.from(innov));
  const thr = jumpK * innovStd;
  const jumpEps: number[] = [], diffEps: number[] = [];
  for (const e of innov) (Math.abs(e) > thr ? jumpEps : diffEps).push(e);
  ...
  for (let k = 0; k < Ydaily.length; k++) Ydaily[k] = Y[k * PERIODS_PER_DAY]!;
...
export function simulatePaths(
  model: PriceModel, state0: ModelState, horizon: number, nPaths: number, seed = 1,
): { paths: Float64Array; times: Float64Array } {
  ...
  for (let h = 0; h < horizon; h++) times[h] = state0.lastEpochMs + (h + 1) * stepMs;
  ...
  for (let pth = 0; pth < nPaths; pth++) {
    ...
    for (let h = 0; h < horizon; h++) {
      ...
      for (let j = 0; j < nj; j++) jump += model.jumpMean + model.jumpStd * gauss();
...
export function forwardCurve(paths: Float64Array, nPaths: number, horizon: number): Float64Array {
  const out = new Float64Array(horizon);
  for (let h = 0; h < horizon; h++) {
    let s = 0;
    for (let p = 0; p < nPaths; p++) s += paths[p * horizon + h]!;
    out[h] = s / nPaths;
  }
  return out;
}
 ...
 */
export function averagePriceVol(
  model: PriceModel, state0: ModelState, horizon: number, nPaths: number, seed = 7,
): { vol: number; forward: number } {
...
export function pathQuantile(paths: Float64Array, nPaths: number, horizon: number, q: number): Float64Array {
  const out = new Float64Array(horizon);
  const col = new Float64Array(nPaths);
  for (let h = 0; h < horizon; h++) {
    for (let p = 0; p < nPaths; p++) col[p] = paths[p * horizon + h]!;
...
// --- helpers ---
function ar1(s: ArrayLike<number>): { phi: number; sigma: number } {
  let sxy = 0, sxx = 0, m = 0;
  for (let i = 1; i < s.length; i++) {
    const a = s[i]!, b = s[i - 1]!;
    if (isNum(a) && isNum(b)) { sxy += a * b; sxx += b * b; m++; }
  }
  const phi = sxx ? sxy / sxx : 0;
  let se = 0, k = 0;
  for (let i = 1; i < s.length; i++) {
    const a = s[i]!, b = s[i - 1]!;
    if (isNum(a) && isNum(b)) { const e = a - phi * b; se += e * e; k++; }
  }
  return { phi, sigma: k ? Math.sqrt(se / k) : 0 };
}

function lastFinite(a: ArrayLike<number>): number {
  for (let i = a.length - 1; i >= 0; i--) if (isNum(a[i]!)) return i;
  return a.length - 1;
}
